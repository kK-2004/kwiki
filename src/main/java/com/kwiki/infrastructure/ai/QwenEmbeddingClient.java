...
/**
 * Qwen text-embedding-v4 adapter over the OpenAI-compatible /embeddings endpoint
 * using the kwiki-specific credential. Requests are batched, retried only for
 * transient statuses (429/5xx) within the deadline, and every returned vector is
 * dimension-checked before use. Errors never include the API key.
 */
@Component
public class QwenEmbeddingClient implements ChunkEmbeddingPort {
    ...
    public QwenEmbeddingClient(@Qualifier("kwikiEmbeddingWebClient") WebClient webClient,
    ...
    private List<float[]> embedBatch(List<String> batch) {
        ...
        for (int attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (!isTransient(e.getStatusCode().value())) {
                    throw new IllegalStateException("embedding request rejected: "
                            + e.getStatusCode().value(), null);
                }
                lastTransient = e;
        ...
        throw new IllegalStateException("embedding retries exhausted after transient failures",
                lastTransient);
    }
