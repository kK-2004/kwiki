...
import com.kwiki.rag.routing.RoutingConfiguration;
...
/**
 * Golden-stream contract: progress events precede tokens, one citations event
 * follows tokens, exactly one terminal done (or error) event ends the stream,
 * sequences are monotonic, no-evidence short-circuits without a provider call,
 * provider failure produces a sanitized terminal error, and client cancellation
 * cancels the provider stream.
 */
class AgenticAnswerStreamTest {
    ...
    private AgenticAnswerService service(ChildRecallPort bm25, ChildRecallPort vector,
        ...
        var scopeResolver = new AuthorizationScopeResolver(memberRepo(), scopeVersions,
                new ScopeCache(cacheProvider(), Duration.ofSeconds(60)));
        return new AgenticAnswerService(
                new RuleFirstRouter(com.kwiki.rag.routing.KeywordRuleSet.defaults(),
                        llmProvider(null), false, metrics),
                new QueryRewriteOrchestrator(new RewriteDecisionService(),
                        ...
                        metrics),
                new HybridRetrievalOrchestrator(embeddings(),
                ...
                llm, scopeResolver, new ChatPersistenceService(nullProvider()), metrics);
    }
    ...
    private static ParentEvidenceResolver.ParentChunkFetcher parentFetcher() {
        return (keys, filter) -> keys.stream()
                .map(key -> new ParentEvidenceChunk(key, 1L, "PAGE", 7L, 3L, "部署",
                        "kwiki 连接外部 MySQL、Redis、内容中心与 Elasticsearch，绝不自建容器。", 0.5,
                        List.of(hit("C1", key))))
                .toList();
    }
    ...
    void successfulStreamFollowsTheGoldenEventOrder() {
        List<ChatStreamEvent> events = collect(service(
                branchWithHits(), branchWithHits(), parentFetcher(),
                prompt -> Flux.just("kwiki ", "连接外部服务。"))
        ...
        assertThat(payloadCitations.get(0)).containsEntry("childChunkKey", "C1")
                .containsEntry("parentChunkKey", "P0")
                .containsKey("charStart").containsKey("excerpt");
    }
    ...
    void noEvidenceShortCircuitsWithoutCallingTheProvider() {
        ...
        List<ChatStreamEvent> events = collect(service(
                emptyBranch(), emptyBranch(), parentFetcher(),
                prompt -> {
        ...
        List<String> types = events.stream().map(ChatStreamEvent::type).toList();
        assertThat(types).contains("retrieve").doesNotContain("token", "citations");
    ...
    void providerFailureTerminatesWithSingleSanitizedErrorAndNoDone() {
        List<ChatStreamEvent> events = collect(service(
                branchWithHits(), branchWithHits(), parentFetcher(),
                prompt -> Flux.error(new IllegalStateException("qwen quota exhausted")))
                .answer(ADMIN, "什么是 kwiki 的部署方式"));
    ...
    void clientCancellationCancelsTheProviderStream() throws Exception {
        ...
        Flux<String> providerFlux = providerSink.asFlux()
                .doOnCancel(() -> cancelled.set(true));

        AgenticAnswerService service = service(branchWithHits(), branchWithHits(),
                parentFetcher(), prompt -> providerFlux);
    ...
    void eventsAreMonotonicPerRequestWithStableRequestId() {
        ...
        assertThat(events).allSatisfy(event -> assertThat(event.requestId())
                .isEqualTo(requestId));
    }
    ...
    void wireFormatIncludesSeqRequestIdAndType() {
        ...
        assertThat(wire).contains("\"seq\":3").contains("\"requestId\":\"req-1\"")
                .contains("\"type\":\"token\"").contains("\"text\":\"你好\"")
                .endsWith("\n\n");
    }
}
