...
/**
 * In-process end-to-end flow with fakes: published Markdown → structured parse →
 * parent/child chunks → child-only dual recall → RRF → distinct parent context →
 * SSE answer with child citations, plus OCR-required rejection before indexing and
 * access revocation aborting generation.
 */
class EndToEndRetrievalFlowTest {
    ...
    private static final List<ChunkHit> CORPUS = List.of(
            new ChunkHit("PAGE:7:3:P0:C0", "PAGE:7:3:P0", 1L, "PAGE", 7L, 3L,
                    "部署", 0, 60, "kwiki 连接外部 MySQL、Redis、内容中心、Elasticsearch"),
            new ChunkHit("PAGE:8:2:P0:C0", "PAGE:8:2:P0", 1L, "PAGE", 8L, 2L,
                    "权限", 0, 40, "成员角色 OWNER EDITOR VIEWER 控制知识库访问"));

    private AgenticAnswerService service(ChildRecallPort bm25, ChildRecallPort vector,
        ...
        return new AgenticAnswerService(
                ...
                new QueryRewriteOrchestrator(new RewriteDecisionService(),
    ...
    private static ChildRecallPort matchingBothBranches() {
        return (query, vector, filter, topK) -> CORPUS.stream()
    ...
    private static ParentEvidenceResolver.ParentChunkFetcher parentFetcher() {
        return (keys, filter) -> keys.stream()
                .map(key -> new ParentEvidenceChunk(key, 1L, "PAGE", 7L, 3L, "部署",
                        "# 部署\nkwiki 连接外部中间件，绝不创建容器。", 0.4,
                        CORPUS.stream().filter(hit -> hit.parentChunkKey().equals(key)).toList()))
                .toList();
    }

    @Test
    void markdownToSseCitationsFlowsEndToEnd() {
        List<ChatStreamEvent> events = service(matchingBothBranches(), matchingBothBranches(),
                parentFetcher(),
                prompt -> {
                    assertThat(prompt).contains("Question: 什么是 kwiki 的部署方式")
                            .contains("绝不创建容器");
                    return Flux.just("kwiki ", "连接外部中间件。");
                },
    ...
    void accessRevocationDuringGenerationAbortsTheStream() {
        ...
        KnowledgeBaseMemberRepository memberRepo = Mockito.mock(KnowledgeBaseMemberRepository.class);
        Mockito.when(memberRepo.findByUserId(7L)).thenReturn(java.util.List.of(
                new com.kwiki.wiki.domain.KnowledgeBaseMember(1L, 7L,
                        com.kwiki.wiki.access.KnowledgeBaseRole.VIEWER, 1L)));

        ScopeVersionService scopeVersions = new ScopeVersionService(
                StandardTestProperties.nullProvider());
        // scope resolves with version 1; revocation lands mid-flight during retrieval
        ParentEvidenceResolver.ParentChunkFetcher revokingFetcher = (keys, filter) -> {
        ...
        var metrics = new SimpleMeterRegistry();
        var budgets = new RetrievalBudgets(50, 40, 8, 3, 24000, Duration.ofSeconds(5));
        AgenticAnswerService revokingService = new AgenticAnswerService(
                new RuleFirstRouter(KeywordRuleSet.defaults(),
                        StandardTestProperties.nullProvider(), false, metrics),
                new QueryRewriteOrchestrator(new RewriteDecisionService(),
                        new com.kwiki.rag.rewrite.ConversationalRewriter(Optional.empty()),
                        ...
                        metrics),
                new HybridRetrievalOrchestrator(texts -> List.of(new float[]{0.5f}),
                        new ConcurrentRecallService(matchingBothBranches(), matchingBothBranches()),
                        new ParentEvidenceResolver(Optional.of(revokingFetcher)),
                        scopeVersions, budgets),
                new com.kwiki.rag.answer.EvidenceAssembler(budgets),
                prompt -> Flux.just("never used"),
                new AuthorizationScopeResolver(memberRepo, scopeVersions,
                        new com.kwiki.infrastructure.redis.ScopeCache(
                                StandardTestProperties.nullProvider(), Duration.ofSeconds(60))),
                new com.kwiki.rag.answer.ChatPersistenceService(
                        StandardTestProperties.nullProvider()),
        ...
        List<ChatStreamEvent> events = revokingService
