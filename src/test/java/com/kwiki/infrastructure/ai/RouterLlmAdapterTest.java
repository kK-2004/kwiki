...
/**
 * HTTP-stub contract for the router adapter: valid JSON (also fenced) parses,
 * timeouts/invalid JSON/provider errors return empty for deterministic fallback,
 * and requests carry no credential in URLs (only the sanitized header).
 */
class RouterLlmAdapterTest {
    ...
    void start() throws IOException {
        ...
        WebClient webClient = WebClient.builder()
                ...
                .build();
        adapter = new RouterLlmAdapter(webClient, properties(), Duration.ofSeconds(2));
    }
    ...
    private static ExternalServicesProperties properties() {
        return new ExternalServicesProperties(
                ...
                new ExternalServicesProperties.AnswerLlm("http://l/v1", "k", "qwen-max",
    ...
    void validJsonDecisionIsParsed() throws Exception {
        String json = "{\"intent\":\"PROCEDURAL\",\"needsRetrieval\":true,"
    ...
    void markdownFencedJsonIsTolerated() {
        ...
        server.enqueue(new MockResponse().setHeader("Content-Type", "application/json")
                .setBody("{\"choices\":[{\"message\":{\"content\":" + quote(fenced) + "}}]}"));
    ...
    void invalidJsonReturnsEmpty() {
        server.enqueue(new MockResponse().setHeader("Content-Type", "application/json")
                .setBody("{\"choices\":[{\"message\":{\"content\":\"not json at all\"}}]}"));
    ...
    void timeoutReturnsEmpty() {
        server.enqueue(new MockResponse().setBodyDelay(5, java.util.concurrent.TimeUnit.SECONDS)
